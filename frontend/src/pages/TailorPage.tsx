...
export function TailorPage() {
    ...
    const [activeSection, setActiveSection] = useState<'setup' | 'intelligence' | 'design' | 'draft'>('setup');
    const [isEditing, setIsEditing] = useState(false);
    const [editableText, setEditableText] = useState("");
    const [showComparison, setShowComparison] = useState(false);
    const [activeTemplate, setActiveTemplate] = useState(AVAILABLE_TEMPLATES[0].id);
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const res = await authenticatedFetch("/analyze", { method: "POST", body: fd });
            if (!res.ok) {
            ...
            // setTimeout(() => setActiveSection('design'), 3000); // Optional auto-advance or just stay on intelligence
        } catch (err: any) {
    ...
    const handleTailor = async () => {
        ...
        try {
            ...
            const response = await authenticatedFetch("/tailor-stream", { method: "POST", body: fd });
            if (!response.ok) {
    ...
    const handleSaveEdit = async () => {
        ...
        setIsLoading('saving');
        try {
            const res = await authenticatedFetch(`/runs/${tailorResult.run_id}`, {
                method: "PATCH",
    ...
    return (
        <div className="w-full max-w-7xl mx-auto px-6 py-10 pb-32 flex flex-col items-center">
            ...
            {activeSection === 'design' && (
                <div className="w-full max-w-3xl flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-white rounded-[2rem] p-12 border border-slate-200 shadow-xl text-center">
                        <h2 className="text-2xl font-black text-slate-900 mb-2">Choose Your Look</h2>
                        <p className="text-slate-500 mb-8">Select a proven ATS-friendly template for your resume.</p>

                        <TemplateSelector selectedId={activeTemplate} onSelect={setActiveTemplate} />

                        <div className="mt-10 flex justify-center">
                            <button
                                onClick={handleTailor}
                                disabled={!!isLoading}
                                className="w-full md:w-auto h-16 px-12 bg-slate-900 text-white rounded-[2rem] font-black uppercase tracking-[0.2em] hover:bg-violet-600 transition-all flex items-center justify-center gap-3 shadow-xl active:scale-95"
                            >
                                {isLoading === 'tailoring' ? 'Generating...' : 'Generate Resume →'}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="w-full flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-visible">
                        ...
                        <div className="flex-1 w-full gap-6 flex">
                            ...
                            <div className="flex-1 bg-white rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden flex flex-col min-h-[600px] transition-all duration-500">
                                ...
                                <div className={`flex-1 p-12 overflow-y-auto no-scrollbar ${AVAILABLE_TEMPLATES.find(t => t.id === activeTemplate)?.class}`}>
